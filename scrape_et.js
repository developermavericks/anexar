...
async function run() {
  ...
  console.log(`Starting Scrape for The Economic Times journalists... Target: 500 records`);
  ...
  while (hasMore && totalProcessed < targetLimit) {
    console.log(`\n--- Fetching Page ${page} ---`);
    // OutletFilter=18 is The Economic Times on Skribe
    let url = `https://www.goskribe.com/api/v1/journalist-records/get-journalists/?OutletFilter=18&MediaFilter=1&SearchFilter=&pageSize=60`;
    if (page > 1) {
