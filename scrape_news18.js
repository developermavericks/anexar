...
async function run() {
  ...
  console.log(`Starting Scrape for News18 India journalists... Target: 500 records`);
  ...
  while (hasMore && totalProcessed < targetLimit) {
    console.log(`\n--- Fetching Page ${page} ---`);
    // OutletFilter=434 is News18 India on Skribe
    let url = `https://www.goskribe.com/api/v1/journalist-records/get-journalists/?OutletFilter=434&SearchFilter=&pageSize=60`;
    if (page > 1) {
    ...
    for (const searchItem of items) {
      ...
      const publication = extractStr(searchItem.outlets || searchItem.outlet || 'News18 India');
  ...
  console.log(`\nScrape finished! Successfully processed ${totalProcessed} News18 India journalists.`);
  process.exit(0);
}
