...
async function run() {
  ...
  console.log(`Starting Page-Tracked Safe Scrape for YourStory...`);
  ...
  if (!scrapingState['yourstory']) {
    scrapingState['yourstory'] = { lastPageScraped: 0 };
  }
  
  let page = (scrapingState['yourstory'].lastPageScraped || 0) + 1;
  ...
  while (hasMore && totalProcessed < targetLimit) {
    console.log(`\n--- Fetching Page ${page} ---`);
    let url = `https://www.goskribe.com/api/v1/journalist-records/get-journalists/?OutletFilter=2795&SearchFilter=&pageSize=60`;
    if (page > 1) {
    ...
    for (const searchItem of items) {
      ...
      const publication = extractStr(searchItem.outlets || searchItem.outlet || 'YourStory');
    ...
    scrapingState['yourstory'] = { lastPageScraped: page };
    fs.writeFileSync(statePath, JSON.stringify(scrapingState, null, 2));
  ...
  console.log(`\nScrape finished! Successfully processed ${totalProcessed} YourStory journalists.`);
  process.exit(0);
}
