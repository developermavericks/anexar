...
if (!(csDone || csDoneOnce)) {
  ...
  function addPrintStyles() {
    let css = `
      ...
      @media print {
          ...
          /* Hide common layout trash */
          header, footer, nav, aside, iframe, .no-print,
          div[class*="ad-"], div[class*="ads-"], div[class*="sidebar"], 
          .social_icons_mobile_wrapper, .social_icons_list, .social_icons,
          .presentedByLink, .mc-modal-wrap, .mc-modal, .subcp_container,
          .paywall_section, .subscription-banner {
              display: none !important;
          }
          /* Un-truncate and fit content */
          .page_wrapper, .article_wrapper, .content_wrapper, .page_left_wrapper, .article_desc, .container {
  ...
  function injectPrintButton() {
    ...
    btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M19 8H5c-1.66 0-3 1.34-3 3v6h4v4h12v-4h4v-6c0-1.66-1.34-3-3-3zm-3 11H8v-5h8v5zm3-7c-.55 0-1-.45-1-1s.45-1 1-1 1 .45 1 1-.45 1-1 1zm-1-9H6v4h12V3z"/></svg> Save to PDF';
    ...
    window.setTimeout(function () {
      ...
      if (article) {
        ...
        if (article_id_dom) {
          ...
          getExtFetch(url_src, '', {headers: {"app-identifier": "com.news.screens", "device-type": "phone", "x-access-token": x_access_token}}, fix_dowjones_fetch, data_ext_fetch_id++, [article]);
        }
      }
    }, 1000);
  }
      ...
      window.setTimeout(function () {
        ...
        if (article) {
          ...
          if (article_id_dom) {
            ...
            getExtFetch(url_src, '', {headers: {"app-identifier": "http://com.news.screens", "device-type": "phone", "x-access-token": x_access_token}}, fix_dowjones_fetch, data_ext_fetch_id++, [article]);
          }
        }
      }, 2000);
    }
...
else if (matchDomain('moneycontrol.com')) {
  let url = window.location.href;
  let paywall_sel = 'div.paywall_section, section.subcp_container';
  let article_sel = 'div.content_wrapper.arti-flow';
  if (document.querySelector(paywall_sel)) {
    func_post = function() {
      injectPrintButton();
    };
    getArchive(url, paywall_sel, '', article_sel);
...
else if (matchDomain('pymnts.com')) {
  let paywall_form = document.querySelector('.pymnts-article-paywall-form, .pymnts-article-paywall-container');
  let hidden_content = document.getElementById('article-paywall-hidden-content');
  if (hidden_content) {
    hidden_content.style.setProperty('display', 'block', 'important');
    if (paywall_form) {
      paywall_form.style.setProperty('display', 'none', 'important');
    }
  }
  injectPrintButton();
}
        ...
        if (json_url_dom) {
          ...
          if (match) {
            ...
            if (window.navigator.userAgent.toLowerCase().includes('chrome')) {
              ...
              getExtFetch(url_src, '', {headers: cs_param}, fix_dowjones_fetch, data_ext_fetch_id++, [article]);
