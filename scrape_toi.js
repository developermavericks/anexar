...
async function run() {
  ...
  const queryKeyword = 'Times of India';
  console.log(`Starting Scrape for TOI journalists... Target: 500 records`);
  ...
  while (hasMore && totalProcessed < targetLimit) {
    console.log(`\n--- Fetching Page ${page} ---`);
    let url = `https://www.goskribe.com/api/v1/journalist-records/get-journalists/?OutletFilter=294&MediaFilter=1&SearchFilter=&pageSize=60`;
    if (page > 1) {
    ...
    for (const searchItem of items) {
      ...
      const publication = extractStr(searchItem.outlets || searchItem.outlet || 'Times of India');
  ...
  console.log(`\nScrape finished! Successfully processed ${totalProcessed} TOI journalists.`);
  process.exit(0);
}
