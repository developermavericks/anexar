...
async function run() {
  ...
  console.log(`Starting Scrape for The Hindu journalists... Target: 500 records`);
  ...
  while (hasMore && totalProcessed < targetLimit) {
    console.log(`\n--- Fetching Page ${page} ---`);
    // OutletFilter=1326 is The Hindu on Skribe
    let url = `https://www.goskribe.com/api/v1/journalist-records/get-journalists/?OutletFilter=1326&SearchFilter=&pageSize=60`;
    if (page > 1) {
    ...
    for (const searchItem of items) {
      ...
      const publication = extractStr(searchItem.outlets || searchItem.outlet || 'The Hindu');
  ...
  console.log(`\nScrape finished! Successfully processed ${totalProcessed} The Hindu journalists.`);
  process.exit(0);
}
