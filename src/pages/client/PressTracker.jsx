...
import { Search, Newspaper, Calendar, User, Sparkles, Filter, AlertCircle, FileSpreadsheet, X, Eye, ArrowUpDown, FileText } from 'lucide-react';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredPress.length > 0 ? (
                    filteredPress.map((pr) => (
                        <div
                            key={pr.id}
                            onClick={() => (pr.type === 'excel' || pr.type === 'docx') && setSelectedExcelReport(pr)}
                            className={`group relative bg-white dark:bg-slate-955 border border-slate-100 dark:border-slate-900 p-6 rounded-3xl shadow-lg hover:shadow-2xl hover:border-purple-500/30 dark:hover:border-purple-500/30 transition-all flex flex-col justify-between overflow-hidden ${(pr.type === 'excel' || pr.type === 'docx') ? 'cursor-pointer' : ''}`}
                        >
                            <div className={`absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r ${pr.type === 'excel' ? 'from-purple-500 to-indigo-650' : pr.type === 'docx' ? 'from-blue-500 to-cyan-500' : 'from-amber-500 to-amber-600'} opacity-0 group-hover:opacity-100 transition-opacity`}></div>
                            ...
                            <div className="space-y-4">
                                <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    {pr.type === 'docx' && (
                                        <span className="inline-flex items-center gap-1 px-3 py-1 bg-blue-500/10 text-blue-600 dark:text-blue-400 text-[10px] font-extrabold uppercase tracking-widest rounded-full">
                                            Word Document
                                        </span>
                                    )}
                                </div>
                                ...
                                {pr.type === 'excel' ? (
                                    ...
                                    <div className="bg-blue-500/5 dark:bg-blue-500/10 rounded-2xl p-5 border border-blue-500/10 flex items-center justify-between group-hover:bg-blue-500/10 transition-colors">
                                        <div className="flex items-center gap-3">
                                            <FileText className="text-blue-500 animate-pulse" size={24} />
                                            <div>
                                                <p className="text-xs font-black text-slate-800 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-450 transition-colors">
                                                    {pr.fileName}
                                                </p>
                                                <p className="text-[10px] text-slate-400 dark:text-slate-500 font-extrabold uppercase mt-0.5">
                                                    Click to open document reader
                                                </p>
                                            </div>
                                        </div>
