...
async function run() {
  ...
  console.log(`Starting Scrape for Mint journalists... Target: 300 records`);
  ...
  while (hasMore && totalProcessed < targetLimit) {
    console.log(`\n--- Fetching Page ${page} ---`);
    // OutletFilter=287 is Mint on Skribe
    let url = `https://www.goskribe.com/api/v1/journalist-records/get-journalists/?OutletFilter=287&SearchFilter=&pageSize=60`;
    if (page > 1) {
    ...
    for (const searchItem of items) {
      ...
      const publication = extractStr(searchItem.outlets || searchItem.outlet || 'Mint');
  ...
  console.log(`\nScrape finished! Successfully processed ${totalProcessed} Mint journalists.`);
  process.exit(0);
}
