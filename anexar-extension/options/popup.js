...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!clipBtn || !statusBox) return;

  let currentEnv = 'production'; // Set to 'local' for local development testing
  ...
  clipBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const targetApi = currentEnv === 'local' 
      ...
      try {
        const cookieUrl = currentEnv === 'local' ? 'http://localhost:5173' : 'https://anexar-9820c.web.app';
