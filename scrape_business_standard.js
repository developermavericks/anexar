...
async function run() {
  ...
  const queryKeyword = 'Business Standard';
  console.log(`Starting Scrape for Business Standard journalists... Target: 500 records`);
  ...
  while (hasMore && totalProcessed < targetLimit) {
    console.log(`\n--- Fetching Page ${page} ---`);
    // OutletFilter=274 is Business Standard on Skribe
    let url = `https://www.goskribe.com/api/v1/journalist-records/get-journalists/?OutletFilter=274&SearchFilter=&pageSize=60`;
    if (page > 1) {
  ...
  console.log(`\nScrape finished! Successfully processed ${totalProcessed} Business Standard journalists.`);
  process.exit(0);
}
