...
async function run() {
  ...
  if (!jwt) {
    console.error("Error: SKRIBE_JWT_TOKEN not found in .env");
    process.exit(1);
  }
  
  console.log(`Starting Page-Tracked Safe Scrape for Moneycontrol journalists...`);
  ...
  // Read state tracker
  const statePath = path.resolve('scraping_state.json');
  let scrapingState = {};
  if (fs.existsSync(statePath)) {
    try {
      scrapingState = JSON.parse(fs.readFileSync(statePath, 'utf8'));
    } catch (e) {
      console.warn("Could not parse scraping_state.json, starting fresh.");
    }
  }
  
  if (!scrapingState['moneycontrol']) {
    scrapingState['moneycontrol'] = { lastPageScraped: 2 }; // Default to 2 since we already have 146 in DB
  }
  
  let page = (scrapingState['moneycontrol'].lastPageScraped || 0) + 1;
  console.log(`[Page Tracker] Resuming from Page: ${page}`);
  
  let totalProcessed = 0;
  const targetLimit = 60; // Process 60 at a time to be credit-safe
  let hasMore = true;
  
  const journalistsRef = collection(db, "journalists");
  
  // Date range logic for recent articles (last 6 months)
  ...
  // Clean duplicate finder
  const findExistingJournalist = async (name, journalistId, publication) => {
    if (journalistId) {
      const qId = query(journalistsRef, where("journalistId", "==", String(journalistId)));
      const snapId = await getDocs(qId);
      if (!snapId.empty) return snapId.docs[0];
    }
    const qName = query(journalistsRef, where("name", "==", name));
    const snapName = await getDocs(qName);
    if (!snapName.empty) {
      const normPub = publication.trim().toLowerCase();
      for (const d of snapName.docs) {
        const existingPub = String(d.data().publication || '').trim().toLowerCase();
        if (existingPub === normPub || 
            existingPub.includes(normPub) || 
            normPub.includes(existingPub) ||
            /^\d+$/.test(existingPub) || 
            /^\d+$/.test(normPub)) {
          return d;
        }
  ...
  while (hasMore && totalProcessed < targetLimit) {
    console.log(`\n--- Fetching Page ${page} ---`);
    let url = `https://www.goskribe.com/api/v1/journalist-records/get-journalists/?OutletFilter=2420&SearchFilter=&pageSize=60`;
    if (page > 1) {
    ...
    if (items.length === 0) {
      console.log("No more results on this page. Resetting tracker to page 0.");
      scrapingState['moneycontrol'] = { lastPageScraped: 0 };
      fs.writeFileSync(statePath, JSON.stringify(scrapingState, null, 2));
      break;
    }
    
    console.log(`Retrieved ${items.length} journalists from search result page.`);
    ...
    for (const searchItem of items) {
      ...
      const publication = extractStr(searchItem.outlets || searchItem.outlet || 'Moneycontrol');
      
      if (!name || !journalistId) continue;
      
      // --- SAFEGUARD CHECK FIRST ---
      const existingDoc = await findExistingJournalist(name, journalistId, publication);
      if (existingDoc) {
        const docData = existingDoc.data();
        if (docData.email && docData.email.trim() !== '') {
          console.log(`   -> [SKIPPED] ${name} already fully enriched in Firestore (0 credits used).`);
          continue;
        }
      }
      
      // Safety pause every 15 active items to prevent detection/flags
      if (totalProcessed > 0 && totalProcessed % 15 === 0) {
        console.log(`\n[Anti-Ban Protection] Pausing for 45 seconds...`);
        await sleep(45000);
      }

      console.log(`[${totalProcessed + 1}/${targetLimit}] Fetching profile details: ${name} (ID: ${journalistId})`);
      
      // Delay before detail fetch
      await sleep(1500);
      ...
      try {
        ...
        if (resDetail.status === 200) {
          ...
          if (resDetail.status === 401 || resDetail.status === 403 || resDetail.status === 429) {
            console.error(`CRITICAL: API returned status ${resDetail.status}. Terminating immediately to protect account!`);
            process.exit(1);
          }
        }
      } catch (err) {
      ...
      // Delay before articles fetch
      await sleep(1500);
      
      let articles = [];
      try {
        const articlesUrl = `https://www.goskribe.com/api/v1/smart-profiles/journalist-portfolio-by-jid?Jid=${journalistId}&pageSize=10&CurrentPage=1&StartDate=${startDate}&EndDate=${endDate}`;
        const resArticles = await fetch(articlesUrl, { headers });
        if (resArticles.status === 200) {
          const articlesJson = await resArticles.json();
          const rawArticles = articlesJson?.data || [];
          if (Array.isArray(rawArticles)) {
            articles = rawArticles.map(art => ({
              title: art.vchtitle || '',
              url: art.vchurl || '',
          ...
          console.warn(`   -> WARNING: Failed to fetch articles (status: ${resArticles.status})`);
          if (resArticles.status === 401 || resArticles.status === 403 || resArticles.status === 429) {
            console.error(`CRITICAL: Articles API returned status ${resArticles.status}. Terminating immediately to protect account!`);
            process.exit(1);
          }
        }
      } catch (err) {
        console.warn(`   -> WARNING: Articles fetch error: ${err.message}`);
      }
      ...
      const docData = {
        ...
        country,
        photo,
        articles, // Save the recent 10 articles!
        updatedAt: new Date().toISOString()
      };

      try {
        if (existingDoc) {
          await updateDoc(doc(db, "journalists", existingDoc.id), docData);
          console.log(`   -> SUCCESS: Updated existing: ${name} (added ${articles.length} articles)`);
        } else {
          await addDoc(journalistsRef, {
            ...docData,
            createdAt: new Date().toISOString()
          });
          console.log(`   -> SUCCESS: Added new: ${name} (added ${articles.length} articles)`);
        }
        totalProcessed++;
      } catch (err) {
        console.error(`   -> Failed writing journalist ${name}:`, err.message);
      }
    }
    
    // Save current progress state
    scrapingState['moneycontrol'] = { lastPageScraped: page };
    fs.writeFileSync(statePath, JSON.stringify(scrapingState, null, 2));

    page++;
    await sleep(2000); // polite page break
  }
  
  console.log(`\nScrape finished! Successfully processed ${totalProcessed} Moneycontrol journalists.`);
  process.exit(0);
}
