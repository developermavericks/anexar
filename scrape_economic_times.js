...
async function run() {
  ...
  console.log(`Starting Page-Tracked Safe Scrape for The Economic Times...`);
  ...
  if (!scrapingState['economic_times']) {
    scrapingState['economic_times'] = { lastPageScraped: 3 }; // Starts on Page 4 since we already have 179 in DB
  }
  
  let page = (scrapingState['economic_times'].lastPageScraped || 0) + 1;
  console.log(`[Page Tracker] Resuming from Page: ${page}`);
  
  let totalProcessed = 0;
  const targetLimit = 60; // Safe batch limit
  let hasMore = true;
  ...
  while (hasMore && totalProcessed < targetLimit) {
    console.log(`\n--- Fetching Page ${page} ---`);
    let url = `https://www.goskribe.com/api/v1/journalist-records/get-journalists/?OutletFilter=18&SearchFilter=&pageSize=60`;
    if (page > 1) {
    ...
    if (items.length === 0) {
      ...
      scrapingState['economic_times'] = { lastPageScraped: 0 };
    ...
    for (const searchItem of items) {
      ...
      const publication = extractStr(searchItem.outlets || searchItem.outlet || 'The Economic Times');
    ...
    scrapingState['economic_times'] = { lastPageScraped: page };
    fs.writeFileSync(statePath, JSON.stringify(scrapingState, null, 2));
  ...
  console.log(`\nScrape finished! Successfully processed ${totalProcessed} Economic Times journalists.`);
  process.exit(0);
}
